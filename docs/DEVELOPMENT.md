...
- 신규 아이템 추가 피해는 `combat/CombatEngine`을 사용하고 피해 종류와 넉백 정책을 명시합니다.
...
업데이트 크기와 관계없이 완성·검증된 작업 단위마다 커밋·푸시합니다. 푸시 전 빌드 성공, 변경 파일, 비밀 정보와 대용량 생성물 포함 여부를 확인합니다. `build/`, `.gradle/`, 개인 월드와 커스포지 프로필은 커밋하지 않습니다.
